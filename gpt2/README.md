...
A self-contained, **network-isolated** container that serves OpenAI's
[GPT-2](https://huggingface.co/gpt2) for local text generation. Weights are
baked into the image at build time; the container runs with **no internet
access**.
