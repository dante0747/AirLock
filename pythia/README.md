...
EleutherAI's Pythia suite, designed for interpretability research. Weights are baked into the image at build time; the container runs with **no internet access**.
...
docker build -t airlock-pythia ./pythia
```
...
```bash
docker pull <DOCKERHUB_USERNAME>/airlock-pythia:latest
docker run -d --name pythia --network none <DOCKERHUB_USERNAME>/airlock-pythia:latest
```
