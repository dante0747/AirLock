...
# ⚖️ StableLM 2 · offline

Stability AI's compact, multilingual StableLM 2 base model, in a self-contained
...
DOCKER_BUILDKIT=1 docker build \
  ...
  -t airlock-stablelm2 ./stablelm/stablelm2
```

> [!TIP]
> Need a credential-free smoke test? Override the model:
> `docker build --build-arg MODEL_ID=gpt2 -t airlock-stablelm2 ./stablelm/stablelm2`
...
```bash
docker run -d --name stablelm --network none airlock-stablelm2
...
docker run -d --name stablelm --network llm-net -p 8000:8000 airlock-stablelm2
...
```bash
docker pull ghcr.io/dante0747/airlock-stablelm2:latest
docker run -d --name stablelm --network none ghcr.io/dante0747/airlock-stablelm2:latest
```
