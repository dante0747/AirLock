...
MiniMax-M3 ships a custom Mixture-of-Experts architecture, so the model is
loaded with trust_remote_code=True. This is safe here: download_model.py already
baked the modeling code into the image at build time, and with HF_HUB_OFFLINE
set transformers executes that cached code WITHOUT fetching anything.
...
model = AutoModelForCausalLM.from_pretrained(
    MODEL_ID, cache_dir=CACHE_DIR, low_cpu_mem_usage=True, trust_remote_code=True
)
model.eval()
