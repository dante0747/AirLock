...
A self-contained, **network-isolated** container that serves a Llama-family
chat model for local text generation. Weights are baked into the image at build
time; the container runs with **no internet access**.
...
> ⚠️ 7B weights are ~13–15 GB. Ensure you have the disk/RAM for it.
...
The HuggingFace token (for gated weights) is passed as a **BuildKit secret**, so
it is used only during the download step and is never persisted in an image
