...
docker build -t airlock-zephyr ./zephyr
```
...
```bash
docker pull <DOCKERHUB_USERNAME>/airlock-zephyr:latest
docker run -d --name zephyr --network none <DOCKERHUB_USERNAME>/airlock-zephyr:latest
```
